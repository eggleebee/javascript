...
|   |  호이스팅 | 함수이름  | 스코프  |
|--------|---|---|---|
| 함수 선언식  | 호이스팅되어 함수 전체가 스코프의 최상단으로 끌어올려진다.   |  이름을 가질 수 있으며, 이 이름은 함수 내부에서만 접근 가능. |  함수 또는 전역 스코프에서 정의.  |
| 함수 표현식  |  변수 호이스팅 규칙을 따르므로 호이스팅되지 않는다.  |  이름을 가질 수도 있고(기명 함수), 없을 수도 있다(익명 함수).|  변수의 스코프 규칙을 따른다. |
